...
function GearScore_OnEvent(GS_Nil, GS_EventName, GS_Prefix, GS_AddonMessage, GS_Whisper, GS_Sender)
	...
	if ( GS_EventName == "PLAYER_EQUIPMENT_CHANGED" ) then
	    local MyGearScore, MyDummy, myIlvl = GearScore_GetScore(UnitName("player"), "player");
		local Red, Blue, Green = GearScore_GetQuality(MyGearScore)
		local iRed, iBlue, iGreen = GearScore_GetQuality(6000/284*myIlvl)
    	PersonalGearScore:SetText(MyGearScore); PersonalGearScore:SetTextColor(Red, Green, Blue, 1)
    	PersonaliLvl:SetText(myIlvl); PersonaliLvl:SetTextColor(iRed, iGreen, iBlue, 1)
  	end
...
function GearScore_GetScore(Name, Target)
	if ( UnitIsPlayer(Target) ) then
		...
		local GearScore = 0; local PVPScore = 0; local ItemCount = 0; local LevelTotal = 0; local TitanGrip = 1; local TempEquip = {}; local TempPVPScore = 0; local iLvlTotal = 0
	...
	return floor(GearScore), floor(LevelTotal/ItemCount), floor(iLvlTotal/16)
	end
end
...
function GearScore_GetItemScore(ItemLink)
    ...
    if ( GS_ItemTypes[ItemEquipLoc] ) then
		...
		if ( ItemRarity >= 2 ) and ( ItemRarity <= 4 )then
            ...
            local iRed, iGreen, iBlue = GearScore_GetQuality(6000/284*((floor(((ItemLevel - Table[ItemRarity].A) / Table[ItemRarity].B) * 1 * Scale)) * 11.25) )
			...
			return GearScore, ItemLevel, GS_ItemTypes[ItemEquipLoc].ItemSlot, Red, Green, Blue, PVPScore, ItemEquipLoc, percent, iRed, iGreen, iBlue ;
		end
  	end
  	return -1, ItemLevel, 50, 1, 1, 1, PVPScore, ItemEquipLoc, 1, 1, 1, 1
end
-------------------------------------------------------------------------------
...
function GearScore_HookSetUnit(arg1, arg2)
	...
	if ( CanInspect("mouseover") ) and ( UnitName("mouseover") == Name ) and not ( GS_PlayerIsInCombat ) then 
 	...
 	if ( MouseOverGearScore ) and ( MouseOverGearScore > 0 ) and ( GS_Settings["Player"] == 1 ) then 
		...
		local iRed, iBlue, iGreen = GearScore_GetQuality(6000/284*MouseOverAverage)
		if ( GS_Settings["Level"] == 1 ) then 
			GameTooltip:AddDoubleLine("iLvl: "..MouseOverAverage, "GS: "..MouseOverGearScore, iRed, iGreen, iBlue, Red, Green, Blue)
		else
...
function GearScore_HookItem(ItemName, ItemLink, Tooltip)
	...
	local ItemScore, ItemLevel, EquipLoc, Red, Green, Blue, PVPScore, ItemEquipLoc, enchantPercent, iRed, iGreen, iBlue = GearScore_GetItemScore(ItemLink);
 	if ( ItemScore >= 0 ) then
		if ( GS_Settings["Item"] == 1 ) then
  			if ( ItemLevel ) and ( GS_Settings["Level"] == 1 ) then Tooltip:AddDoubleLine("iLvl "..ItemLevel, "GS: "..ItemScore,Red, Blue, Green, Red, Blue, Green);
...
PaperDollFrame:CreateFontString("PersonalGearScoreLabel")
PersonalGearScoreLabel:SetFont("Fonts\\FRIZQT__.TTF", 10)
PersonalGearScoreLabel:SetText("GS:")
PersonalGearScoreLabel:SetPoint("BOTTOMLEFT",PaperDollFrame,"TOPLEFT",70,-265)
PersonalGearScoreLabel:Show()
...
PersonalGearScore:SetPoint("BOTTOMRIGHT",PaperDollFrame,"TOPRIGHT",-262,-265)
PersonalGearScore:Show()

PaperDollFrame:CreateFontString("PersonaliLvlLabel")
PersonaliLvlLabel:SetFont("Fonts\\FRIZQT__.TTF", 10)
PersonaliLvlLabel:SetText("iLvl:")
PersonaliLvlLabel:SetPoint("BOTTOMLEFT",PaperDollFrame,"TOPLEFT",70,-253)
PersonaliLvlLabel:Show()

PaperDollFrame:CreateFontString("PersonaliLvl")
PersonaliLvl:SetFont("Fonts\\FRIZQT__.TTF", 10)
PersonaliLvl:SetText("iLvl: 0")
PersonaliLvl:SetPoint("BOTTOMRIGHT",PaperDollFrame,"TOPRIGHT",-262,-253)
PersonaliLvl:Show()
